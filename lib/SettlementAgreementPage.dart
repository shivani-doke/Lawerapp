...
class SettlementAgreementPage extends StatefulWidget {
  const SettlementAgreementPage({super.key});

  @override
  State<SettlementAgreementPage> createState() => _SettlementAgreementPageState();
}

class _SettlementAgreementPageState extends State<SettlementAgreementPage> {
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('settlement_agreement');
  ...
  void _applyClientToSettlementParty({
  ...
  String? _settlementClientAssignmentLabel({
    ...
    for (final entry in _linkedClientIdsByGroup.entries) {
      for (var rowIndex = 0; rowIndex < entry.value.length; rowIndex++) {
        ...
        return entry.key == 'party_a_details' ? 'Party A Details' : 'Party B Details';
      }
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel =
                                    _settlementClientAssignmentLabel(
                                  clientId: clientId,
    ...
    final assignmentLabel = _settlementClientAssignmentLabel(
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'settlement_agreement');
      setState(() {
