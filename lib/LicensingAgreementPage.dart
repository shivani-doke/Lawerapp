...
class LicensingAgreementPage extends StatefulWidget {
  const LicensingAgreementPage({super.key});

  @override
  State<LicensingAgreementPage> createState() => _LicensingAgreementPageState();
}

class _LicensingAgreementPageState extends State<LicensingAgreementPage> {
  ...
  String? _linkedLicensorClientName;
  String? _linkedLicenseeClientName;
  String? _linkedLicensorClientId;
  String? _linkedLicenseeClientId;
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('licensing_agreement');
  ...
  void _applyClientToLicensingParty({
    ...
    setState(() {
      if (party == 'licensor') {
        _setFieldText('licensor_name', name);
        _setFieldText('licensor_address', address);
        _linkedLicensorClientName = name;
        _linkedLicensorClientId = (client['id'] ?? '').toString();
      } else if (party == 'licensee') {
        _setFieldText('licensee_name', name);
        _setFieldText('licensee_address', address);
        _linkedLicenseeClientName = name;
        _linkedLicenseeClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _licensingClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'licensor' && _linkedLicensorClientId == clientId) {
      return 'Licensor';
    }
    if (currentParty != 'licensee' && _linkedLicenseeClientId == clientId) {
      return 'Licensee';
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel =
                                    _licensingClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    final assignmentLabel = _licensingClientAssignmentLabel(
    ...
    _applyClientToLicensingParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'licensing_agreement');
      setState(() {
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'licensor_name' || field.name == 'licensee_name';
    final linkedClientName = field.name == 'licensor_name'
        ? _linkedLicensorClientName
        : field.name == 'licensee_name'
            ? _linkedLicenseeClientName
            : null;
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party:
                        field.name == 'licensor_name' ? 'licensor' : 'licensee',
                    title: field.name == 'licensor_name'
                        ? 'Autofill Licensor Details'
                        : 'Autofill Licensee Details',
                  ),
