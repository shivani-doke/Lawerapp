...
class AdoptionPapersPage extends StatefulWidget {
  const AdoptionPapersPage({super.key});

  @override
  State<AdoptionPapersPage> createState() => _AdoptionPapersPageState();
}

class _AdoptionPapersPageState extends State<AdoptionPapersPage> {
  ...
  String? _linkedParent1ClientName;
  String? _linkedParent2ClientName;
  String? _linkedParent1ClientId;
  String? _linkedParent2ClientId;
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('adoption_papers');
  ...
  void _applyClientToAdoptionParty({
    ...
    setState(() {
      if (party == 'parent1') {
        _setFieldText('adoptive_parent1_name', name);
        _setFieldText('adoptive_parent1_age', age);
        _setFieldText('adoptive_parent1_address', address);
        _linkedParent1ClientName = name;
        _linkedParent1ClientId = (client['id'] ?? '').toString();
      } else if (party == 'parent2') {
        _setFieldText('adoptive_parent2_name', name);
        _setFieldText('adoptive_parent2_age', age);
        _setFieldText('adoptive_parent2_address', address);
        _linkedParent2ClientName = name;
        _linkedParent2ClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _adoptionClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'parent1' && _linkedParent1ClientId == clientId) {
      return 'Adoptive Parent 1';
    }
    if (currentParty != 'parent2' && _linkedParent2ClientId == clientId) {
      return 'Adoptive Parent 2';
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                      ...
                      child: filteredClients.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel = _adoptionClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    _applyClientToAdoptionParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'adoption_papers');
      setState(() {
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill = field.name == 'adoptive_parent1_name' ||
        field.name == 'adoptive_parent2_name';
    final linkedClientName = field.name == 'adoptive_parent1_name'
        ? _linkedParent1ClientName
        : field.name == 'adoptive_parent2_name'
            ? _linkedParent2ClientName
            : null;
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party: field.name == 'adoptive_parent1_name' ? 'parent1' : 'parent2',
                    title: field.name == 'adoptive_parent1_name'
                        ? 'Autofill Adoptive Parent 1 Details'
                        : 'Autofill Adoptive Parent 2 Details',
                  ),
