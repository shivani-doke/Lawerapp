...
class NonCompeteAgreementPage extends StatefulWidget {
  const NonCompeteAgreementPage({super.key});

  @override
  State<NonCompeteAgreementPage> createState() => _NonCompeteAgreementPageState();
}

class _NonCompeteAgreementPageState extends State<NonCompeteAgreementPage> {
  ...
  String? _linkedRestrictedClientName;
  String? _linkedBeneficiaryClientName;
  String? _linkedRestrictedClientId;
  String? _linkedBeneficiaryClientId;
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('non_compete_agreement');
  ...
  void _applyClientToNonCompeteParty({
    ...
    setState(() {
      if (party == 'restricted') {
        _setFieldText('restricted_party_name', name);
        _setFieldText('restricted_party_address', address);
        _linkedRestrictedClientName = name;
        _linkedRestrictedClientId = (client['id'] ?? '').toString();
      } else if (party == 'beneficiary') {
        _setFieldText('beneficiary_party_name', name);
        _setFieldText('beneficiary_party_address', address);
        _linkedBeneficiaryClientName = name;
        _linkedBeneficiaryClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _nonCompeteClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'restricted' && _linkedRestrictedClientId == clientId) {
      return 'Restricted Party';
    }
    if (currentParty != 'beneficiary' && _linkedBeneficiaryClientId == clientId) {
  ...
  Future<void> _showClientAutofillDialog({
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel =
                                    _nonCompeteClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    final assignmentLabel = _nonCompeteClientAssignmentLabel(
    ...
    _applyClientToNonCompeteParty(party: party, client: selectedClient);
  }
  ...
  bool _evaluateSingleShowIfCondition(
    String condition, {
    DocumentField? groupField,
    int? rowIndex,
  }) {
  ...
  bool _isFieldVisible(
    ...
    final orClauses = condition.split('||');
    for (final orClause in orClauses) {
      final andClauses = orClause
          .split('&&')
          .map((part) => part.trim())
      ...
      final allMatched = andClauses.every(
        (part) => _evaluateSingleShowIfCondition(
          part,
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'non_compete_agreement');
      setState(() {
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill = field.name == 'restricted_party_name' ||
        field.name == 'beneficiary_party_name';
    final linkedClientName = field.name == 'restricted_party_name'
        ? _linkedRestrictedClientName
        : field.name == 'beneficiary_party_name'
            ? _linkedBeneficiaryClientName
            : null;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party: field.name == 'restricted_party_name'
                        ? 'restricted'
                        : 'beneficiary',
                    title: field.name == 'restricted_party_name'
                        ? 'Autofill Restricted Party Details'
                        : 'Autofill Beneficiary Party Details',
                  ),
          ...
          const SizedBox(height: 8),
          input,
          if (field.name == 'restriction_period_months')
            _buildRestrictionPeriodWarning(),
        ],
      ),
    );
  }
  ...
  Widget _buildRestrictionPeriodWarning() {
    final rawValue =
        _fieldControllers['restriction_period_months']?.text.trim() ?? '';
    final months = int.tryParse(rawValue);
    if (months == null || months <= 12) {
      return const SizedBox.shrink();
    }

    final message = months > 24
        ? 'Warning: restriction periods above 24 months are often heavily scrutinized and may be difficult to enforce in many jurisdictions.'
        : 'Warning: restriction periods above 12 months may face enforceability challenges depending on the jurisdiction and commercial context.';
    ...
    return const Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text(
            "Fill in the details on the left, then click\n\"Generate Document\" to create your Non-Compete Agreement.",
            textAlign: TextAlign.center,
            style: TextStyle(color: Colors.grey),
          ),
        ],
