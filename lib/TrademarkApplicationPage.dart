...
class TrademarkApplicationPage extends StatefulWidget {
  const TrademarkApplicationPage({super.key});

  @override
  State<TrademarkApplicationPage> createState() => _TrademarkApplicationPageState();
}

class _TrademarkApplicationPageState extends State<TrademarkApplicationPage> {
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('trademark_application');
  ...
  String? _trademarkClientAssignmentLabel({
  ...
  Future<void> _showClientAutofillDialog({
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final clientId =
                                    (client['id'] ?? '').toString();
                                final assignedLabel =
                                    _trademarkClientAssignmentLabel(
                                  clientId: clientId,
    ...
    final assignedLabel = _trademarkClientAssignmentLabel(
  ...
  void _initializeGroupField(DocumentField field) {
    ...
    if (field.name == 'applicant_details') {
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'trademark_application');
      setState(() {
  ...
  Widget _buildGroupField(DocumentField field) {
    ...
    final supportsClientAutofill = field.name == 'applicant_details';
