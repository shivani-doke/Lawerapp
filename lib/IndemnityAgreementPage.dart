...
class IndemnityAgreementPage extends StatefulWidget {
  const IndemnityAgreementPage({super.key});

  @override
  State<IndemnityAgreementPage> createState() => _IndemnityAgreementPageState();
}

class _IndemnityAgreementPageState extends State<IndemnityAgreementPage> {
  ...
  String? _linkedIndemnifierClientName;
  String? _linkedIndemnifiedClientName;
  String? _linkedIndemnifierClientId;
  String? _linkedIndemnifiedClientId;
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('indemnity_agreement');
  ...
  void _applyClientToIndemnityParty({
    ...
    setState(() {
      if (party == 'indemnifier') {
        _setFieldText('indemnifier_name', name);
        _setFieldText('indemnifier_address', address);
        _linkedIndemnifierClientName = name;
        _linkedIndemnifierClientId = (client['id'] ?? '').toString();
      } else if (party == 'indemnified') {
        _setFieldText('indemnified_party_name', name);
        _setFieldText('indemnified_party_address', address);
        _linkedIndemnifiedClientName = name;
        _linkedIndemnifiedClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _indemnityClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'indemnifier' && _linkedIndemnifierClientId == clientId) {
      return 'Indemnifier';
    }
    if (currentParty != 'indemnified' && _linkedIndemnifiedClientId == clientId) {
      return 'Indemnified Party';
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                      ...
                      child: filteredClients.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel = _indemnityClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    _applyClientToIndemnityParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'indemnity_agreement');
      setState(() {
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'indemnifier_name' || field.name == 'indemnified_party_name';
    final linkedClientName = field.name == 'indemnifier_name'
        ? _linkedIndemnifierClientName
        : field.name == 'indemnified_party_name'
            ? _linkedIndemnifiedClientName
            : null;
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party: field.name == 'indemnifier_name' ? 'indemnifier' : 'indemnified',
                    title: field.name == 'indemnifier_name'
                        ? 'Autofill Indemnifier Details'
                        : 'Autofill Indemnified Party Details',
                  ),
