...
class ChildCustodyAgreementPage extends StatefulWidget {
  const ChildCustodyAgreementPage({super.key});

  @override
  State<ChildCustodyAgreementPage> createState() => _ChildCustodyAgreementPageState();
}

class _ChildCustodyAgreementPageState extends State<ChildCustodyAgreementPage> {
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('child_custody_agreement');
  ...
  void _applyClientToCustodyParty({
    ...
    setState(() {
      if (party == 'parent1') {
        _setFieldText('parent1_name', name);
        _setFieldText('parent1_address', address);
        _linkedParent1ClientName = name;
        _linkedParent1ClientId = (client['id'] ?? '').toString();
      } else if (party == 'parent2') {
        _setFieldText('parent2_name', name);
        _setFieldText('parent2_address', address);
  ...
  String? _custodyClientAssignmentLabel({
  ...
  Future<void> _showClientAutofillDialog({
                      ...
                      child: filteredClients.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel = _custodyClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    _applyClientToCustodyParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'child_custody_agreement');
      setState(() {
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'parent1_name' || field.name == 'parent2_name';
    final linkedClientName = field.name == 'parent1_name'
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party: field.name == 'parent1_name' ? 'parent1' : 'parent2',
                    title: field.name == 'parent1_name'
                        ? 'Autofill Parent 1 Details'
                        : 'Autofill Parent 2 Details',
                  ),
