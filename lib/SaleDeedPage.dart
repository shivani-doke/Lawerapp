...
class _SaleDeedPageState extends State<SaleDeedPage> {
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('sale_deed');
  ...
  String? _saleClientAssignmentLabel({
    ...
    for (final entry in _linkedClientIdsByGroup.entries) {
      for (var rowIndex = 0; rowIndex < entry.value.length; rowIndex++) {
        ...
        final partyLabel =
            entry.key == 'seller_details' ? 'Seller' : 'Buyer';
  ...
  Future<void> _showClientAutofillDialog({
    ...
    final assignedLabel = _saleClientAssignmentLabel(
  ...
  void _initializeGroupField(DocumentField field) {
    ...
    if (field.name == 'seller_details' || field.name == 'buyer_details') {
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'sale_deed');
      setState(() {
  ...
  Widget _buildGroupField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'seller_details' || field.name == 'buyer_details';
