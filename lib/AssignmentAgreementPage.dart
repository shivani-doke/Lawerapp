...
class AssignmentAgreementPage extends StatefulWidget {
  const AssignmentAgreementPage({super.key});

  @override
  State<AssignmentAgreementPage> createState() => _AssignmentAgreementPageState();
}

class _AssignmentAgreementPageState extends State<AssignmentAgreementPage> {
  ...
  String? _linkedAssignorClientName;
  String? _linkedAssigneeClientName;
  String? _linkedAssignorClientId;
  String? _linkedAssigneeClientId;
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('assignment_agreement');
  ...
  void _applyClientToAssignmentParty({
    ...
    setState(() {
      if (party == 'assignor') {
        _setFieldText('assignor_name', name);
        _setFieldText('assignor_address', address);
        _linkedAssignorClientName = name;
        _linkedAssignorClientId = (client['id'] ?? '').toString();
      } else if (party == 'assignee') {
        _setFieldText('assignee_name', name);
        _setFieldText('assignee_address', address);
        _linkedAssigneeClientName = name;
        _linkedAssigneeClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _assignmentClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'assignor' && _linkedAssignorClientId == clientId) {
      return 'Assignor';
    }
    if (currentParty != 'assignee' && _linkedAssigneeClientId == clientId) {
      return 'Assignee';
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel =
                                    _assignmentClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    final assignmentLabel = _assignmentClientAssignmentLabel(
    ...
    _applyClientToAssignmentParty(party: party, client: selectedClient);
  }
  ...
  bool _matchesShowIfCondition(
    ...
    final actualNumber = double.tryParse((actualValue ?? '').toString().trim());
    final expectedNumber = double.tryParse(expectedToken.trim());
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'assignment_agreement');
      setState(() {
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'assignor_name' || field.name == 'assignee_name';
    final linkedClientName = field.name == 'assignor_name'
        ? _linkedAssignorClientName
        : field.name == 'assignee_name'
            ? _linkedAssigneeClientName
            : null;
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party:
                        field.name == 'assignor_name' ? 'assignor' : 'assignee',
                    title: field.name == 'assignor_name'
                        ? 'Autofill Assignor Details'
                        : 'Autofill Assignee Details',
                  ),
