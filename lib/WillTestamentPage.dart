...
class _WillTestamentPageState extends State<WillTestamentPage> {
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('will_and_testament');
  ...
  void _applyClientToWillParty({
    ...
    setState(() {
      if (party == 'testator') {
        ...
        _linkedExecutorClientId = (client['id'] ?? '').toString();
      }
    });
  }
  ...
  Future<void> _showClientAutofillDialog({
    ...
    _applyClientToWillParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'will_and_testament');
      setState(() {
