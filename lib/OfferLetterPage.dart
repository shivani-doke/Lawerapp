...
class _OfferLetterPageState extends State<OfferLetterPage> {
  ...
  String? _linkedCompanyClientName;
  String? _linkedCandidateClientName;
  String? _linkedCompanyClientId;
  String? _linkedCandidateClientId;
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('offer_letter');
  ...
  void _applyClientToOfferParty({
    ...
    setState(() {
      if (party == 'company') {
        _setFieldText('company_name', name);
        _setFieldText('company_address', address);
        _linkedCompanyClientName = name;
        _linkedCompanyClientId = (client['id'] ?? '').toString();
      } else if (party == 'candidate') {
        _setFieldText('candidate_name', name);
        _setFieldText('candidate_address', address);
        _linkedCandidateClientName = name;
        _linkedCandidateClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _offerClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'company' && _linkedCompanyClientId == clientId) {
      return 'Company';
    }
    if (currentParty != 'candidate' && _linkedCandidateClientId == clientId) {
      return 'Candidate';
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                      ...
                      child: filteredClients.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel = _offerClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    _applyClientToOfferParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'offer_letter');
      setState(() {
  ...
  bool _isOfferExpired() {
    final validTill =
        _parseFormDate(_fieldControllers['offer_valid_till']?.text ?? '');
    if (validTill == null) return false;
    final today = DateTime.now();
    final normalizedToday = DateTime(today.year, today.month, today.day);
    return validTill.isBefore(normalizedToday);
  }

  String? _validateOfferLetterRules() {
    if (_isOfferExpired() && (_boolValues['candidate_acceptance'] ?? false)) {
      return 'Candidate Acceptance cannot be marked after the Offer Valid Till date has passed.';
    }
    return null;
  }
  ...
  Future<void> _generateDocument() async {
    ...
    final validationError = _validateOfferLetterRules();
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'company_name' || field.name == 'candidate_name';
    final linkedClientName = field.name == 'company_name'
        ? _linkedCompanyClientName
        : field.name == 'candidate_name'
            ? _linkedCandidateClientName
            : null;
    ...
    switch (field.type) {
      case 'dropdown':
        ...
        break;
      case 'boolean':
        final isCandidateAcceptanceField = field.name == 'candidate_acceptance';
        input = Container(
          decoration: BoxDecoration(
          ...
          child: CheckboxListTile(
            ...
            onChanged: isCandidateAcceptanceField && _isOfferExpired()
                ? null
                : (selected) {
            ...
            title: Text(
              isCandidateAcceptanceField && _isOfferExpired()
                  ? '${field.hint ?? 'Yes'} (disabled because the offer has expired)'
                  : (field.hint ?? 'Yes'),
            ),
            dense: true,
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party: field.name == 'company_name' ? 'company' : 'candidate',
                    title: field.name == 'company_name'
