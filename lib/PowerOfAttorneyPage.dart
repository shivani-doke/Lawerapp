...
class _PowerOfAttorneyPageState extends State<PowerOfAttorneyPage> {
  ...
  String? _linkedPrincipalClientName;
  String? _linkedAgentClientName;
  String? _linkedPrincipalClientId;
  String? _linkedAgentClientId;
  ...
  final Map<String, List<DocumentField>> _powerSpecificFields = {
    'Banking': [
      DocumentField(
        name: 'bank_name',
        label: 'Bank Name',
      ...
      DocumentField(
        ...
        type: 'number',
        hint: 'Enter transaction limit, if any',
        required: false,
      ),
    ],
    'Financial': [
      DocumentField(
        ...
        type: 'textarea',
        hint: 'Describe financial powers granted',
        required: false,
      ),
    ],
    'Legal': [
      DocumentField(
        name: 'legal_authority_scope',
        label: 'Legal Authority Scope',
        type: 'textarea',
        hint: 'Describe legal powers granted',
        required: false,
      ),
    ],
    'Business': [
      ...
      DocumentField(
        name: 'business_authority_scope',
        label: 'Business Authority Scope',
        type: 'textarea',
        hint: 'Describe business powers granted',
        required: false,
      ),
    ],
    'Documentation': [
      DocumentField(
        name: 'document_authority_scope',
        label: 'Document Authority Scope',
        type: 'textarea',
        hint: 'Describe document execution powers granted',
        required: false,
      ),
    ],
  };
  ...
  void _applyClientToPoaParty({
    ...
    setState(() {
      if (party == 'principal') {
        _setFieldText('principal_name', name);
        _setFieldText('principal_age', age);
        _setFieldText('principal_occupation', occupation);
        _setFieldText('principal_address', address);
        _setFieldText('principal_id_proof', idProof);
        _linkedPrincipalClientName = name;
        _linkedPrincipalClientId = (client['id'] ?? '').toString();
      } else if (party == 'agent') {
        _setFieldText('agent_name', name);
        _setFieldText('agent_age', age);
        _setFieldText('agent_occupation', occupation);
        _setFieldText('agent_address', address);
        _setFieldText('agent_id_proof', idProof);
        _linkedAgentClientName = name;
        _linkedAgentClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _poaClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'principal' && _linkedPrincipalClientId == clientId) {
      return 'Principal';
    }
    if (currentParty != 'agent' && _linkedAgentClientId == clientId) {
      return 'Agent';
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                      ...
                      child: filteredClients.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final clientId =
                                    (client['id'] ?? '').toString();
                                final assignmentLabel = _poaClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    _applyClientToPoaParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      ...
      if (autoSelectFirst &&
        ...
        final firstRef = _savedReferences.first;
        final firstSubtype = firstRef['subtype']?.toString();
        if (firstSubtype != null && firstSubtype.isNotEmpty) {
          _selectedPoaSubtype = firstSubtype;
        }
        final autoSelectedId = firstRef['id']?.toString();
  ...
  Future<void> _loadPoaSubtypes() async {
    try {
      final subtypes =
          await ApiService().getDocumentSubtypes(documentType: 'power_of_attorney');
      if (!mounted) return;

      if (subtypes.isNotEmpty) {
        setState(() {
          _poaSubtypes = subtypes;
          _selectedPoaSubtype ??= subtypes.first['id'];
        });
      }
    } catch (_) {
      // Fallback silently; backend may not provide subtypes.
    }
  }
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'principal_name' || field.name == 'agent_name';
    final linkedClientName = field.name == 'principal_name'
        ? _linkedPrincipalClientName
        : field.name == 'agent_name'
            ? _linkedAgentClientName
            : null;
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party:
                        field.name == 'principal_name' ? 'principal' : 'agent',
                    title: field.name == 'principal_name'
                        ? 'Autofill Principal Details'
                        : 'Autofill Agent Details',
                  ),
  ...
  List<DocumentField> _getVisibleFields() {
    ...
    if (_selectedPoaSubtype == null || _selectedPoaSubtype == 'general_poa') {
      final selectedPowers = _multiselectValues['general_powers'] ?? <String>{};
      for (final power in selectedPowers) {
        final extras = _powerSpecificFields[power] ?? const <DocumentField>[];
        for (final extra in extras) {
          fieldsByName.putIfAbsent(extra.name, () => extra);
        }
      }
    }
  ...
  List<Widget> _buildVisibleFields() {
    ...
    for (final field in _getVisibleFields()) {
