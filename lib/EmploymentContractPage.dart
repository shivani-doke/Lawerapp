...
class EmploymentContractPage extends StatefulWidget {
  const EmploymentContractPage({super.key});

  @override
  State<EmploymentContractPage> createState() => _EmploymentContractPageState();
}

class _EmploymentContractPageState extends State<EmploymentContractPage> {
  ...
  String? _linkedEmployerClientName;
  String? _linkedEmployeeClientName;
  String? _linkedEmployerClientId;
  String? _linkedEmployeeClientId;
  ...
  void initState() {
    ...
    _openedFromUploads =
        UploadNavigationContext.consumeReferenceOnlyMode('employment_contract');
  ...
  void _applyClientToEmploymentParty({
    ...
    setState(() {
      if (party == 'employer') {
        _setFieldText('employer_name', name);
        _setFieldText('employer_address', address);
        _linkedEmployerClientName = name;
        _linkedEmployerClientId = (client['id'] ?? '').toString();
      } else if (party == 'employee') {
        _setFieldText('employee_name', name);
        _setFieldText('employee_address', address);
        _setFieldText('employee_id_proof', idProof);
        _linkedEmployeeClientName = name;
        _linkedEmployeeClientId = (client['id'] ?? '').toString();
      }
    });
  }

  String? _employmentClientAssignmentLabel({
    required String clientId,
    required String currentParty,
  }) {
    if (clientId.isEmpty) return null;
    if (currentParty != 'employer' && _linkedEmployerClientId == clientId) {
      return 'Employer';
    }
    if (currentParty != 'employee' && _linkedEmployeeClientId == clientId) {
      return 'Employee';
    }
    return null;
  }
  ...
  Future<void> _showClientAutofillDialog({
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final assignmentLabel =
                                    _employmentClientAssignmentLabel(
                                  clientId: clientId,
                                  currentParty: party,
                                );
    ...
    final assignmentLabel = _employmentClientAssignmentLabel(
    ...
    _applyClientToEmploymentParty(party: party, client: selectedClient);
  }
  ...
  Future<void> _loadSavedReferences({bool autoSelectFirst = true}) async {
    ...
    try {
      final refs =
          await ApiService().listReferences(documentType: 'employment_contract');
      setState(() {
  ...
  Widget _buildField(DocumentField field) {
    ...
    final supportsClientAutofill =
        field.name == 'employer_name' || field.name == 'employee_name';
    final linkedClientName = field.name == 'employer_name'
        ? _linkedEmployerClientName
        : field.name == 'employee_name'
            ? _linkedEmployeeClientName
            : null;
              ...
              if (supportsClientAutofill)
                TextButton.icon(
                  onPressed: () => _showClientAutofillDialog(
                    party:
                        field.name == 'employer_name' ? 'employer' : 'employee',
                    title: field.name == 'employer_name'
                        ? 'Autofill Employer Details'
